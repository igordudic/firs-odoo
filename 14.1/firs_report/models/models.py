...
class firsConfig(models.Model):
    ...
    business_device = fields.Char("Business Device",
                                  help="Serial number of POS device in a business place (given after registration)")
    active_till = fields.Datetime("Active Till")
    auth_type = fields.Char("Auth Type")
    auth_token = fields.Char("Auth Token")

    inv_session_id = fields.Integer("Business Device",
                                    help="Serial number of POS device in a business place (given after registration)")
...
class accountInvoice(models.Model):
    ...
    def get_firs_report_url(self, invoice):
        if (invoice.sk_uid):
            ...
            url += conf_info.vat_number + "~" + str(invoice.id) + "~" + conf_info.inv_business_place + "~" + str(
                conf_info.inv_session_id) + "~" + date_invoice + "~" + str(amount_total) + "~" + str(invoice.sk_sid)
        else:
    ...
    #     # val = conf.client_secret + conf.vat_number + conf.business_place + currentOrder.pos_session_id.toString() + bill_number + crr + tot;
    #     conf = self.env['firs.config'].search([])
    #     bill_number = fields.datetime.now()
    #
    #     date = datetime.now()
    #     crr = date.isoformat()
    #     new_date = crr.split(".")
    ...
    #     # val= conf.client_secret + conf.vat_number + conf.business_place + str(self.id) + str(self.amount_total) + bill_number + crr
    #     val = str(self.id) + str(self.amount_total) + str(bill_number) + crr
    #     hash_object = hashlib.md5(val.encode())
    #     hash_value = hash_object.hexdigest()
    #     self.sk_sid = hash_value
    #     self.receipt_seq = 'INv/Businessplace/deviceID/billno/sessionId'
    #     print(hash_value, "md5")
    #     return res
